# ZTF Classifier — Project Continuity Handbook v1.8

## 1. Canonical state

- Repository: `saeed92m/ztf-classifier`
- Default branch: `main`
- Current main development head: `827304eb85e29516c6ad59ca5eb0bccc273a7948`
- Immutable scientific baseline: tag `v0.2.0`
- Python: 3.11.16
- Runtime: Ubuntu 26.04 LTS under WSL2
- Environment: pyenv + `.venv`
- Handbook v1.8 supersedes v1.7 for current project continuity.
...
## 25. Unified Scientific Feature Engine

The scientific feature-generation boundary is now implemented as a backend-neutral engine.

The engine contract accepts normalized observations plus explicit parameters and returns a deterministic feature vector with:

- feature schema version;
- backend identity;
- backend software version;
- extraction parameters;
- SHA-256 of the normalized input observation table.

The current `native` backend delegates to the existing frozen v0.2 extractor. The canonical dataset builder now routes feature extraction through this engine, so the production path has a stable integration point for future native and SCoPe-compatible backends.

The v0.2 scientific baseline remains unchanged: the same 42 features and deterministic order are preserved.

## 26. Current implementation checkpoint

- Web/API foundation: merged.
- Scientific Workbench foundation: merged.
- Real source-backed observation API: merged in PR #20 at `579e75a3743d8803042a364d263b95c3aa66b8c8`.
- Unified Scientific Feature Engine: merged in PR #21 at `827304eb85e29516c6ad59ca5eb0bccc273a7948`.
- SCoPe-compatible feature backend: not yet implemented.
- Persistent jobs/database: not yet implemented.

Next implementation slice: add the first observation-to-feature API/application workflow using the unified engine, then add a SCoPe-compatible backend adapter and scientific feature provenance in the Workbench.
