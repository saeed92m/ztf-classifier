...
- Handbook v2.0 supersedes v1.9 for current project continuity.
- Software milestone `v0.4.0` is the immutable release target at `457b887b1a08ede4fdce3f2ca2e3ce6c2b3a47d8`; current `main` contains later continuity and hardening commits.
...
- The project is in the integrated Astronomical Data Analysis & Discovery Platform completion phase; scientific external-validation evidence is now a mandatory final gate.
...
The remaining acceptance boundary is cross-layer evidence: post-merge CI/security verification, external scientific benchmark execution, immutable raw-cache reproduction where required, and final acceptance evidence.
...
1. green full CI and CodeQL on the merged main lineage;
...
8. final documentation, changelog, and scientific-validation evidence alignment;

The authoritative acceptance checklist is docs/PRODUCTION_ACCEPTANCE_CHECKLIST.md.


## 31. Scientific Validation & External Benchmark Gate — mandatory product requirement

Scientific validation against independent external evidence is now a mandatory product-completion requirement.

The product must not be declared scientifically final based only on internal unit/integration tests, model agreement, or performance on training-derived data.

The permanent validation contract is documented in:
docs/SCIENTIFIC_VALIDATION_BENCHMARK.md

The validation architecture has five evidence layers:

1. independent labeled benchmarks;
...
5. OOD and explicit failure-case validation.

The benchmark framework must keep ground truth separate from reference predictions. Agreement with ALeRCE or another classifier is not itself ground truth.

Initial external benchmark sources to evaluate include:

- StarEmbed ZTF_40k, including its train/validation/test and OOD/anomaly splits;
- the 730,184-object ZTF periodic-variable classification release with SIMBAD cross-match;
- the historical 781,602-object ZTF periodic-variable catalog;
- a pinned, versioned ZTF DR24 source-backed subset;
- ALeRCE public API/TAP data for independent reference comparisons.

Current external-source facts and versions must be recorded in benchmark manifests rather than embedded as undocumented assumptions.

### Mandatory benchmark controls

Every release candidate affecting scientific behavior must evaluate the relevant benchmark suite and record:
...
- train/validation/test/OOD split definition;
- object-level leakage exclusions;
- preprocessing and feature-generation contract;
- model/software/data provenance;
- reference-system provenance;
- per-object predictions;
- per-class metrics and confusion matrix;
- probabilistic calibration diagnostics where applicable;
- OOD diagnostics;
- failure cases;
- reproducibility evidence.

Large external datasets must remain outside Git. Git stores manifests, checksums, benchmark definitions, reference metadata, and reports.

### Scientific release gate

The production acceptance checklist now requires an explicit Scientific Validation Gate.

The gate cannot be marked complete until:

1. pinned benchmark manifests exist;
2. independent label provenance is documented;
3. leakage checks pass;
4. benchmark execution succeeds;
5. classification/probabilistic/OOD metrics are generated as applicable;
6. independent reference-system comparisons are separated from ground truth;
7. failure and edge cases are evaluated;
8. benchmark results are reproducible;
9. regressions are investigated and resolved or explicitly dispositioned;
10. the final evidence is attached to the release acceptance record.

This gate is a permanent part of the Definition of Done for the final product and future scientific releases.
