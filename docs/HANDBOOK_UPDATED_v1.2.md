# ZTF Classifier — Project Continuity Handbook v1.2

## 1. Canonical state

- Repository: `saeed92m/ztf-classifier`
- Default branch: `main`
- Current merged production baseline: `3a7d87639a4635820b2773c46d1159426fcdb389`
- Immutable scientific baseline: tag `v0.2.0`
- Python: 3.11.16
- Runtime: Ubuntu 26.04 LTS under WSL2
- Environment: pyenv + `.venv`
- Handbook v1.2 supersedes v1.1 for current project continuity.
...
CI run #84 passed before merge.

PR #11 merged as:

`3a7d87639a4635820b2773c46d1159426fcdb389`
...
Known authoritative full-suite baseline before the final E2E documentation/merge sequence:
...
## 9. Release-readiness phase

The core scientific and production inference path is implemented.

Remaining release work should concentrate on operational completion rather than rebuilding the model pipeline:

1. finalize current documentation and operator-facing usage;
2. verify clean main and reproducible installation;
3. perform final end-to-end release validation;
4. define the next public/project version separately from immutable scientific baseline `v0.2.0`;
5. prepare release metadata and changelog;
6. create the production release only after the final gate passes.

No claim of final release readiness should be made until these checks are actually executed.
