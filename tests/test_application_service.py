...
from ztf_classifier.application.schemas import PredictionRequest, PredictionResponse
...
def test_prediction_request_accepts_valid_request(tmp_path: Path) -> None:
    dataset = pd.DataFrame({"feature": [1.0, 2.0]})
    request = PredictionRequest(dataset=dataset, artifact_dir=tmp_path)
    assert request.dataset is dataset
    assert request.artifact_dir == tmp_path.resolve()


def test_prediction_request_rejects_non_dataframe(tmp_path: Path) -> None:
    with pytest.raises(TypeError, match="pandas DataFrame"):
        PredictionRequest(dataset=[1.0, 2.0], artifact_dir=tmp_path)  # type: ignore[arg-type]


def test_prediction_request_rejects_missing_artifact_directory(tmp_path: Path) -> None:
    missing = tmp_path / "missing"
    with pytest.raises(ValueError, match="does not exist"):
        PredictionRequest(dataset=pd.DataFrame(), artifact_dir=missing)


def test_prediction_request_rejects_file_as_artifact_directory(tmp_path: Path) -> None:
    artifact_file = tmp_path / "artifact"
    artifact_file.write_text("not a directory", encoding="utf-8")
    with pytest.raises(ValueError, match="not a directory"):
        PredictionRequest(dataset=pd.DataFrame(), artifact_dir=artifact_file)
...
def test_predict_dataframe_matches_request_based_api(
    ...
    dataset = pd.DataFrame({"feature": [1.0, 2.0]})
    response = ApplicationService().predict_dataframe(dataset, tmp_path)
