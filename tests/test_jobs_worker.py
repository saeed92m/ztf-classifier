...
def test_job_store_rejects_terminal_state_regression_and_terminal_replacement(
    tmp_path: Path,
) -> None:
    store = JobStore(tmp_path / "jobs.sqlite3")
    ...
    with pytest.raises(ValueError, match="Invalid job transition"):
        store.transition(job.job_id, status="failed")
...
def test_job_store_rejects_running_to_running_transition(tmp_path: Path) -> None:
    store = JobStore(tmp_path / "jobs.sqlite3")
    store.create(oid="ZTF-transition-running", survey="ztf", model_version=None)
    running = store.claim_next()
    assert running is not None

    with pytest.raises(ValueError, match="Invalid job transition"):
        store.transition(running.job_id, status="running")
