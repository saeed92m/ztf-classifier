...
def _open_text(path: Path, member: str | None = None, *, required_columns: Iterable[str] = ()):
    if path.suffix.lower() == ".zip":
        zf = zipfile.ZipFile(path)
        names = [name for name in zf.namelist() if not name.endswith("/")]
        if member is None:
            if len(names) != 1:
                candidates = [name for name in names if _zip_member_matches_header(zf, name, required_columns)]
                if len(candidates) == 1:
                    member = candidates[0]
                else:
                    zf.close()
                    if not candidates:
                        raise ValueError(f"{path} contains {len(names)} files; no member matches required columns {tuple(required_columns)!r}")
                    raise ValueError(f"{path} has multiple members matching required columns {tuple(required_columns)!r}: {tuple(candidates)!r}; specify member explicitly")
            else:
                member = names[0]
...
def _iter_rows(path: Path, member: str | None = None, *, required_columns: Iterable[str] = ()) -> Iterator[dict[str, str]]:
    ...
    try:
        ...
        if "\t" in header_line:
            ...
            parse = lambda value: [item.strip() for item in next(csv.reader([value], delimiter=","))]
        else:
            parse = lambda value: value.split()
        header = header_tokens or parse(header_line)
        for line in lines:
            if not line.strip():
                continue
            stripped = line.strip()
...
def _qualified_ids(path: Path, *, sigma: float, parent_ids: set[str], member: str | None, error_columns: Iterable[str]) -> set[str]:
    rows = _iter_rows(path, member, required_columns=("SourceID", *error_columns))
    first = next(rows, None)
    if first is None:
        return set()
...
def derive_730k(parent_path: Path, g_lightcurve: Path, r_lightcurve: Path, output: Path, *, config: DerivationConfig = _DEFAULT_DERIVATION_CONFIG, parent_member: str | None = None, g_member: str | None = None, r_member: str | None = None, expected_parent_rows: int = 781_602, expected_selected_rows: int = 730_184, parent_evidence_sha256: str | None = None, code_version: str = "working-tree") -> DerivationResult:
    ...
    g_ids = _qualified_ids(g_lightcurve, sigma=config.g_sigma, parent_ids=parent_ids, member=g_member, error_columns=("e_gmag", "magerr", "mag_err", "error"))
    r_ids = _qualified_ids(r_lightcurve, sigma=config.r_sigma, parent_ids=parent_ids, member=r_member, error_columns=("e_rmag", "magerr", "mag_err", "error"))
    ...
    write_evidence_manifest(output.with_name(output.name + ".evidence.json"), benchmark_id="ztf_periodic_730k", source_id="ztf_periodic_730k", acquisition_timestamp=datetime.now(UTC).isoformat(), code_version=code_version, artifacts=(EvidenceArtifact.from_path(output, "derived_benchmark_snapshot"),), parent_evidence_sha256=parent_evidence_sha256, query_manifest_sha256=hashlib.sha256(selection_manifest).hexdigest())
    output.with_name(output.name + ".derivation.json").write_text(json.dumps(result.to_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return result
